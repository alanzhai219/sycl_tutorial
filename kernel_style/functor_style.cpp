...
class vector_add {
public:
    vector_add(float* arg_a, float* arg_b, float* arg_c, size_t arg_N)
        : a(arg_a), b(arg_b), c(arg_c), n(arg_N) {}
    void operator()(sycl::nd_item<3> ndi) const { // attension: must be "const" function
...
int main() {
    ...
    q.parallel_for(parallel_size, vector_add(a_dev, b_dev, c_dev, N));
    q.wait();
