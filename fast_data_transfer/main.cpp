...
static const char *usage_str =
    ...
    "\n                            [default: 2^28]"
    "\n  -p                       use SYCL prepare_for_device_copy/release_from_device_copy APIs"
    "\n                            [default: disabled]"
...
int main(int argc, char **argv) {
  ...
  sycl::queue dq;
  sycl::device dev = dq.get_device();
  size_t max_compute_units = dev.get_info<sycl::info::device::max_compute_units>();
  auto BE = dq.get_device().template get_info<sycl::info::device::opencl_c_version>().empty()
                ? "L0"
                : "OpenCL";
  ...
  if (prepare) {
    ...
    sycl::ext::oneapi::experimental::prepare_for_device_copy(hostp, transfer_upper_limit, dq);
  }

  void *destp = malloc_device<char>(transfer_upper_limit, dq.get_device(), dq.get_context());
  dq.memset(destp, 2, transfer_upper_limit);
  dq.wait();
