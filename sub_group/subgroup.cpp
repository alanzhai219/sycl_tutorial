...
void Add(sycl::queue &q, std::vector<int> &a, std::vector<int> &b, std::vector<int> &c) {
    ...
    int* d_a = sycl::malloc_device<int>(a.size(), q);
    int* d_b = sycl::malloc_device<int>(b.size(), q);
    int* d_c = sycl::malloc_device<int>(c.size(), q);

    q.copy(a.data(), d_a, a.size()).wait();
    q.copy(b.data(), d_b, b.size()).wait();
    q.copy(c.data(), d_c, c.size()).wait();

    q.submit([&](sycl::handler &cgh) {
        auto out = sycl::stream(10240, 7680, cgh);

        sycl::range<3> globalSize(sequence_length, head_num, head_size); // (4,4,2)
        sycl::range<3> workGroupSize(sequence_length, head_num / 2, head_size); // (4,2,2)

        cgh.parallel_for<class kernelAdd>(
            sycl::nd_range(globalSize, workGroupSize),
            [=](sycl::nd_item<3> item) [[intel::reqd_sub_group_size(8)]] {
                addKernel(out, item, d_a, d_b, d_c);
        });
    }).wait();

    q.copy(d_c, c.data(), c.size()).wait();
